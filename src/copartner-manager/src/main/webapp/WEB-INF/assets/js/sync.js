 ...
 * var PARAMS = {};
 * PARAMS["id"] = 123;
 * $.tulipSync.send({action : "/user/profile.htm", parameters : PARAMS, method : "POST"});
 * 
 * @author 刘飞
 */
(function($) {
	$.tulipSync = {
		send : function(options) {
			...
			var syncPostForm = document.createElement("form");
			syncPostForm.action = action;
			syncPostForm.method = method;
			syncPostForm.style.display = "none";
			for ( var p in parameters) {
				var opt = document.createElement("input");
				opt.name = p;
				opt.value = encodeURIComponent(parameters[p]);
				opt.type = "hidden";
				syncPostForm.appendChild(opt);
			}
			document.body.appendChild(syncPostForm);
			syncPostForm.submit();
		}
	}
})(jQuery);
