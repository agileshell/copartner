		...
		<div class="mainbar">
			<div class="page-head">
				...
				<div class="bread-crumb pull-right">
					<a href="/article/list"><i class="icon-home"></i>首页</a><span class="divider">/</span>文章列表
				</div>
									...
									<table class="table table-striped table-bordered table-hover">
										...
										<tbody>
											<c:if test="${!success}">
												...
												<tr><td colspan="6" style="text-align: center;"><a class="btn btn-default btn-sm" href="/article/create">新建报表</a></td></tr>
											</c:if>
											<c:if test="${success}">
												<c:forEach var="article" items="${articleList}" varStatus="status">
													<tr>
														...
														<td>
															...
															<c:if test="${article.status == 2}"> 审核通过 </c:if>
															<c:if test="${article.status == 3}"> 已发布 </c:if>
															<c:if test="${article.status == 4}"> 审核未通过 </c:if>
														</td>
														<td>${article.created}</td>
														<td>
															<div class="btn-group">
																<a class="btn btn-xs btn-default" href="detail/${article.id}">
																	详情
																</a>
															</div>
														</td>
